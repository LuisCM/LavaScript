...
#define EXPECT_CHILD_AT_INDEX(node, childIndex)                              \
    do                                                                       \
    {                                                                        \
    	LAVASCRIPT_ASSERT((node) != nullptr);                                      \
        if ((node)->getChild((childIndex)) == nullptr)                       \
        {                                                                    \
        	LAVASCRIPT_INTERNAL_EXCEPTION("expected node for child index " +       \
                                    toString((childIndex)));                 \
        }                                                                    \
    } while (0)
...
#define EXPECT_NUM_CHILDREN(node, expected)                                  \
    ...
    {                                                                        \
        ...
        {                                                                    \
            ...
            {                                                                \
            	LAVASCRIPT_INTERNAL_EXCEPTION("expected " + toString((expected)) + \
...
#define EXPECT_SYMBOL(node)                                                  \
    ...
    {                                                                        \
        ...
        {                                                                    \
        	LAVASCRIPT_INTERNAL_EXCEPTION("expected symbol for node " +            \
...
static IntermediateInstr * newInstruction(LSCompiler & compiler,
		const OpCode op) {
...
static IntermediateInstr * newInstruction(LSCompiler & compiler,
		const OpCode op, const Symbol * symbol, const Variant::Type dataType,
		const UInt16 stackIndex) {
	IntermediateInstr * instr = compiler.instrPool.allocate();
	instr->next = nullptr;
...
static IntermediateInstr * newInstruction(LSCompiler & compiler,
		const OpCode op, const IntermediateInstr * jumpTarget) {
	IntermediateInstr * instr = compiler.instrPool.allocate();
	instr->next = nullptr;
...
static IntermediateInstr * newInstruction(LSCompiler & compiler,
		const IntermediateInstr * copy) {
	LAVASCRIPT_ASSERT(copy != nullptr);
	IntermediateInstr * instr = compiler.instrPool.allocate();
	*instr = *copy;
	instr->next = nullptr;
	instr->uid = compiler.instrCount++;
...
static IntermediateInstr * linkInstructions(IntermediateInstr * head,
		IntermediateInstr * newTail) {
	LAVASCRIPT_ASSERT(head != nullptr);
...
struct CodeGenContext final {
	...
	// LSVM is needed mainly for the runtime type info tables.
	LSVM * vm = nullptr;
	...
	bool nodeWasVisited(const SyntaxTreeNode * node) const {
		...
		const auto iter = std::find(std::begin(visitedNodes),
				std::end(visitedNodes), node);
	...
	void beginFunction(const IntermediateInstr * endLabel,
			const SyntaxTreeNode * root, LSVarInfoTable & varInfo) {
		if (isInsideFunctionBody()) {
			LAVASCRIPT_INTERNAL_EXCEPTION("nested functions not supported yet");
		}

		returnAnchor = endLabel;
		currFuncNameSymbol = root->symbol;
		currentStackSize = std::max(0,
				varInfo.getFuncStackSize(currFuncNameSymbol) - 1);
	}
...
using EmitInstrForNodeCB = IntermediateInstr * (*)(LSCompiler &, CodeGenContext &, const SyntaxTreeNode *);
static IntermediateInstr * traverseTreeRecursive(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root);

static void findSymbolInfo(LSCompiler & compiler, CodeGenContext & codeGen,
		const Symbol * objSymbol, Variant::Type * outDataType,
		UInt16 * outStackIndex, bool * outIsFuncLocal) {
	const VarInfo * vi;
	if (codeGen.isInsideFunctionBody()) {
		vi = compiler.varInfo.findNamedFuncLocalVar(objSymbol,
				codeGen.currFuncNameSymbol);
		if (vi != nullptr) {
			(*outIsFuncLocal) = true;
	...
	if (vi != nullptr) {
		...
		if (codeGen.forLoopIteratorNode != nullptr
				&& codeGen.forLoopIteratorNode->symbol == objSymbol) {
			(*outDataType) = eval2VarType(
					codeGen.forLoopIteratorNode->evalType);
			if (codeGen.isInsideFunctionBody()) {
				(*outIsFuncLocal) = true;
...
static const TypeId * findTypeNameTypeId(CodeGenContext & codeGen,
		const Symbol * typeSymbol) {
	// Type-Id of a type name.
	LAVASCRIPT_ASSERT(typeSymbol != nullptr); LAVASCRIPT_ASSERT(codeGen.vm != nullptr);
	return codeGen.vm->types.findTypeId(typeSymbol->name);
}

static const TypeId * findObjectTypeId(LSCompiler & compiler,
		CodeGenContext & codeGen, const Symbol * objSymbol) {
...
static IntermediateInstr * makeLoadIntLiteral(LSCompiler & compiler,
		const int value) {
	const Symbol * literalValueSym = compiler.symTable.findOrDefineIntLiteral(
			value);
	return newInstruction(compiler, OpCode::LoadGlobal, literalValueSym,
			Variant::Type::Integer, InvalidStackIndex);
}

static IntermediateInstr * makeLoadMemberOffsetInstr(LSCompiler & compiler,
		CodeGenContext & codeGen, const Symbol * objSymbol,
		const Symbol * memberSymbol, const TypeId ** outTypeId,
		int * outMemOffset) {
	const TypeId * typeId = findObjectTypeId(compiler, codeGen, objSymbol);
	if (typeId == nullptr || typeId->templateObject == nullptr) {
		LAVASCRIPT_INTERNAL_EXCEPTION(
				"'" + toString(objSymbol->name)
						+ "' is not a type or has no dynamic members table!");
	}

	const int memberOffset = typeId->templateObject->findMemberIndex(
			memberSymbol->name);
	if (memberOffset < 0) {
		LAVASCRIPT_INTERNAL_EXCEPTION(
				"object has no member '" + toString(memberSymbol->name) + "'");
	}

	LAVASCRIPT_ASSERT(outTypeId != nullptr); LAVASCRIPT_ASSERT(outMemOffset != nullptr);
...
static IntermediateInstr * appendTrailingLoadOpIfNeeded(LSCompiler & compiler,
		IntermediateInstr * argument, const SyntaxTreeNode * root) {
	if (root->nodeType == SyntaxTreeNode::Type::ExprAssign
		...
		tailLoad->op = (
				last->op == OpCode::StoreGlobal ?
						OpCode::LoadGlobal : OpCode::LoadLocal);
...
static IntermediateInstr * getParamListChain(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	return (root->getChild(0) ?
			traverseTreeRecursive(compiler, codeGen, root->getChild(0)) :
			nullptr);
}

static IntermediateInstr * emitNOOP(LSCompiler & compiler, CodeGenContext &,
		const SyntaxTreeNode *) {
	// For the node types that generate no code.
	return newInstruction(compiler, OpCode::NoOp);
}

static IntermediateInstr * emitProgStart(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	IntermediateInstr * programRoot = newInstruction(compiler,
			OpCode::ProgStart);
	if (root->getChild(0) != nullptr) // A translation unit might be empty.
			{
		IntermediateInstr * translationUnit = traverseTreeRecursive(compiler,
				codeGen, root->getChild(0));
		programRoot = linkInstructions(programRoot, translationUnit);
	}
	return programRoot;
}

static IntermediateInstr * emitStatement(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_CHILD_AT_INDEX(root, 1);
	if (root->getChild(0) != nullptr) {
		IntermediateInstr * lhs = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));
		IntermediateInstr * rhs = traverseTreeRecursive(compiler, codeGen,
				root->getChild(1));
...
static IntermediateInstr * emitIfThen(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// If the then-statement ('if' body) is not empty:
	if (root->getChild(1) != nullptr) {
		EXPECT_NUM_CHILDREN(root, 2);
		IntermediateInstr * condition = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));
		IntermediateInstr * thenStatement = traverseTreeRecursive(compiler,
				codeGen, root->getChild(1));
		IntermediateInstr * noopEnd = newInstruction(compiler, OpCode::NoOp);
		IntermediateInstr * jumpIfFalse = newInstruction(compiler,
				OpCode::JmpIfFalse, noopEnd);

		if (root->getChild(0)->nodeType == SyntaxTreeNode::Type::ExprFuncCall) {
			condition = linkInstructions(condition,
					newInstruction(compiler, OpCode::LoadRVR));
		}
	...
	{
		LAVASCRIPT_ASSERT(root->getChild(0) != nullptr);
		IntermediateInstr * condition = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));
		IntermediateInstr * noopEnd = newInstruction(compiler, OpCode::NoOp);
		IntermediateInstr * jumpIfFalse = newInstruction(compiler,
...
static IntermediateInstr * emitIfThenElse(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	...
	IntermediateInstr * thenStatement = (
			root->getChild(1) ?
					traverseTreeRecursive(compiler, codeGen,
							root->getChild(1)) :
					nullptr);
	IntermediateInstr * elseStatement = (
			root->getChild(2) ?
					traverseTreeRecursive(compiler, codeGen,
							root->getChild(2)) :
					nullptr);
	...
	// Jump labels:
	IntermediateInstr * noopEnd = newInstruction(compiler, OpCode::NoOp);
	IntermediateInstr * jumpToEnd = newInstruction(compiler, OpCode::Jmp,
			noopEnd);
	IntermediateInstr * jumpIfFalse = newInstruction(compiler,
			OpCode::JmpIfFalse,
			(elseStatement != nullptr) ? elseStatement : jumpToEnd);
...
static IntermediateInstr * emitIfThenElseIf(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	...
	if (elseStatement != nullptr) {
		return linkInstructions(condition, jumpIfFalse, thenStatement,
				jumpToEnd, elseStatement, noopEnd);
	} else {
		return linkInstructions(condition, jumpIfFalse, thenStatement,
				jumpToEnd, noopEnd);
	}
...
static IntermediateInstr * emitLoop(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	...
	IntermediateInstr * loopContinue = newInstruction(compiler, OpCode::Jmp,
			noopLabelStart);
	...
	if (root->getChild(0) != nullptr) {
		...
		IntermediateInstr * loopBody = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));

		codeGen.loopStartAnchor = nullptr;
		codeGen.loopEndAnchor = nullptr;

		return linkInstructions(noopLabelStart, loopBody, loopContinue,
				noopLabelEnd);
	}
...
static IntermediateInstr * emitWhileLoop(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// The conditional expression must be present.
	EXPECT_CHILD_AT_INDEX(root, 0);
	IntermediateInstr * loopCond = traverseTreeRecursive(compiler, codeGen,
			root->getChild(0));
	IntermediateInstr * noopLabelEnd = newInstruction(compiler, OpCode::NoOp);
	IntermediateInstr * loopContinue = newInstruction(compiler, OpCode::Jmp,
			loopCond);
	...
	IntermediateInstr * jumpIfFalse = newInstruction(compiler,
			OpCode::JmpIfFalse, noopLabelEnd);
...
static IntermediateInstr * emitForLoop(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	auto makeLoadInstr = [](LSCompiler & comp, const IntermediateInstr * instr)
	{
	...
	auto makeCommonStoreInst =
			[](LSCompiler & comp, const IntermediateInstr * instr)
			{
	...
	EXPECT_CHILD_AT_INDEX(root, 0); EXPECT_CHILD_AT_INDEX(root, 1);

	// The termination condition or collection to iterate over (for-loop parameter):
	IntermediateInstr * forParam = traverseTreeRecursive(compiler, codeGen,
			root->getChild(1));
	if (root->getChild(1)->nodeType == SyntaxTreeNode::Type::ExprFuncCall) {
		forParam = linkInstructions(forParam,
				newInstruction(compiler, OpCode::LoadRVR));
	}
	...
	// 'i' iterator:
	IntermediateInstr * forVars = traverseTreeRecursive(compiler, codeGen,
			root->getChild(0));
	IntermediateInstr * storeIter = forVars;
	for (; storeIter->next != nullptr; storeIter = storeIter->next) {
	} LAVASCRIPT_ASSERT(storeIter->op == OpCode::StoreSetTypeLocal || storeIter->op == OpCode::StoreSetTypeGlobal);

	// The internal index '__for_idx__' is linked to the 'i' iterator:
	forVars = linkInstructions(forVars,
			traverseTreeRecursive(compiler, codeGen,
					root->getChild(0)->getChild(2)));
	IntermediateInstr * storeIdx = forVars;
	for (; storeIdx->next != nullptr; storeIdx = storeIdx->next) {
	} LAVASCRIPT_ASSERT(storeIdx->op == OpCode::StoreSetTypeLocal || storeIdx->op == OpCode::StoreSetTypeGlobal);

	// The copy of the loop parameter/condition '__for_prm__' is linked to '__for_idx__':
	forVars = linkInstructions(forVars,
			traverseTreeRecursive(compiler, codeGen,
					root->getChild(0)->getChild(2)->getChild(2)));
	IntermediateInstr * storeParam = forVars;
	for (; storeParam->next != nullptr; storeParam = storeParam->next) {
	} LAVASCRIPT_ASSERT(storeParam->op == OpCode::StoreSetTypeLocal || storeParam->op == OpCode::StoreSetTypeGlobal);
	...
	forPrep = linkInstructions(forPrep,
			makeCommonStoreInst(compiler, storeIdx));
	forPrep = linkInstructions(forPrep,
			makeCommonStoreInst(compiler, storeIter));
	...
	forStep = linkInstructions(forStep,
			makeCommonStoreInst(compiler, storeIdx));
	forStep = linkInstructions(forStep,
			makeCommonStoreInst(compiler, storeIter));
	...
	// Tie them up:
	IntermediateInstr * temp = linkInstructions(forVars, forParam, forPrmInit,
			forPrep, postPrepJump, forStep, forTest, jumpIfFalse, loopBody); // loopBody may be null, so must be linked at the tail.
	IntermediateInstr * forLoopInstrList = linkInstructions(temp, forCont,
			forEnd);
...
static IntermediateInstr * emitBreak(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode *) {
	LAVASCRIPT_ASSERT(codeGen.loopEndAnchor != nullptr);
	return newInstruction(compiler, OpCode::Jmp, codeGen.loopEndAnchor);
}

static IntermediateInstr * emitContinue(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode *) {
	LAVASCRIPT_ASSERT(codeGen.loopStartAnchor != nullptr);
	return newInstruction(compiler, OpCode::Jmp, codeGen.loopStartAnchor);
}

static IntermediateInstr * emitMatchPrep(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_CHILD_AT_INDEX(root, 0); // The param to match with

	IntermediateInstr * matchParam = traverseTreeRecursive(compiler, codeGen,
			root->getChild(0));
...
static IntermediateInstr * emitMatchTest(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	LAVASCRIPT_ASSERT(codeGen.returnAnchor != nullptr); EXPECT_CHILD_AT_INDEX(root, 0); // The expression to match against.

	IntermediateInstr * caseExpr = traverseTreeRecursive(compiler, codeGen,
			root->getChild(0));
	IntermediateInstr * caseBody = (
			root->getChild(1) ?
					traverseTreeRecursive(compiler, codeGen,
							root->getChild(1)) :
					nullptr);
	IntermediateInstr * caseEnd = newInstruction(compiler, OpCode::NoOp);
	IntermediateInstr * caseEndJump = newInstruction(compiler,
			OpCode::JmpIfFalse, caseEnd);
	IntermediateInstr * matchEndJump = newInstruction(compiler, OpCode::Jmp,
			codeGen.returnAnchor);
	IntermediateInstr * matchTestOp = newInstruction(compiler,
			OpCode::MatchTest);

	IntermediateInstr * matchStmt = linkInstructions(caseExpr, matchTestOp,
			caseEndJump, caseBody, matchEndJump);
	IntermediateInstr * siblingCase = (
			root->getChild(2) ?
					traverseTreeRecursive(compiler, codeGen,
							root->getChild(2)) :
					nullptr);
...
static IntermediateInstr * emitMatchDefault(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
...
static IntermediateInstr * emitFunctionDecl(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_SYMBOL(root); // The function name.

	IntermediateInstr * funcStart = newInstruction(compiler, OpCode::FuncStart,
			root->symbol, Variant::Type::Function, InvalidStackIndex);
	IntermediateInstr * funcEnd = newInstruction(compiler, OpCode::FuncEnd,
			root->symbol, Variant::Type::Function, InvalidStackIndex);
...
static IntermediateInstr * emitReturn(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	LAVASCRIPT_ASSERT(codeGen.returnAnchor != nullptr);
	IntermediateInstr * retJump = newInstruction(compiler, OpCode::JmpReturn,
			codeGen.returnAnchor);

	// Return simply resolves its child expression if it has one.
	if (root->getChild(0) != nullptr) {
		IntermediateInstr * retExpr = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));
		...
		if (root->getChild(0)->nodeType != SyntaxTreeNode::Type::ExprFuncCall) {
			IntermediateInstr * setRVR = newInstruction(compiler,
					OpCode::StoreRVR);
...
static IntermediateInstr * emitEnumDecl(LSCompiler & compiler, CodeGenContext &,
		const SyntaxTreeNode * root) {
	EXPECT_SYMBOL(root);

	const Symbol * enumObjNameSymbol = demangleEnumTypeName(compiler.symTable,
			root->symbol);
	const Symbol * enumTypeNameSymbol = root->symbol;

	IntermediateInstr * loadTypeInsr = newInstruction(compiler,
			OpCode::LoadGlobal, enumTypeNameSymbol, Variant::Type::Tid,
			InvalidStackIndex);
	IntermediateInstr * initInstr = newInstruction(compiler,
			OpCode::EnumDynInit, enumObjNameSymbol, Variant::Type::LSObject,
			InvalidStackIndex);

	return linkInstructions(loadTypeInsr, initInstr);
}

static IntermediateInstr * emitNewRange(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_CHILD_AT_INDEX(root, 1); // Range start
	EXPECT_CHILD_AT_INDEX(root, 2); // Range end

	IntermediateInstr * startExpr = traverseTreeRecursive(compiler, codeGen,
			root->getChild(1));
	IntermediateInstr * endExpr = traverseTreeRecursive(compiler, codeGen,
			root->getChild(2));
	IntermediateInstr * newRangeOp = newInstruction(compiler, OpCode::NewRange);

	return linkInstructions(startExpr, endExpr, newRangeOp,
			getParamListChain(compiler, codeGen, root));
}

static IntermediateInstr * emitArraySubscript(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// child 0 is reserved for parameter lists and may be null.
	EXPECT_CHILD_AT_INDEX(root, 1); EXPECT_CHILD_AT_INDEX(root, 2);

	// If either term is a function call, we must load from the Return Value Register.
	IntermediateInstr * arrayExpr = traverseTreeRecursive(compiler, codeGen,
			root->getChild(1));
	if (root->getChild(1)->nodeType == SyntaxTreeNode::Type::ExprFuncCall) {
		arrayExpr = linkInstructions(arrayExpr,
				newInstruction(compiler, OpCode::LoadRVR));
	}

	IntermediateInstr * subscriptExpr = traverseTreeRecursive(compiler, codeGen,
			root->getChild(2));
	if (root->getChild(2)->nodeType == SyntaxTreeNode::Type::ExprFuncCall) {
		subscriptExpr = linkInstructions(subscriptExpr,
				newInstruction(compiler, OpCode::LoadRVR));
	}

	return linkInstructions(arrayExpr, subscriptExpr,
			newInstruction(compiler, OpCode::ArraySubscript));
}

static IntermediateInstr * emitMemberRef(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// child 0 is reserved for parameter list chains in function/constructor calls.
	EXPECT_CHILD_AT_INDEX(root, 1); EXPECT_CHILD_AT_INDEX(root, 2); EXPECT_SYMBOL(root->getChild(2));

	IntermediateInstr * objLoadInstr = traverseTreeRecursive(compiler, codeGen,
			root->getChild(1));
	...
	if (objSymbol != nullptr) // First pass
			{
		int memberOffset = 0;
		loadMemOffsInstr = makeLoadMemberOffsetInstr(compiler, codeGen,
				objSymbol, memberSymbol, &codeGen.lastMemberTypeId,
				&memberOffset);

		const LSObject::Member & member =
				codeGen.lastMemberTypeId->templateObject->getMemberAt(
						memberOffset);
		if (member.data.type == Variant::Type::LSObject) {
			codeGen.lastMemberTypeId = member.data.value.asObject->getTypeId();
		}
	} else // Going down a chain of obj.member.member.member ...
	{
		LAVASCRIPT_ASSERT(codeGen.lastMemberTypeId != nullptr);
		const int memberOffset =
				codeGen.lastMemberTypeId->templateObject->findMemberIndex(
						memberSymbol->name);

		if (memberOffset < 0) {
			LAVASCRIPT_INTERNAL_EXCEPTION(
					"sub object of type '"
							+ toString(codeGen.lastMemberTypeId->name)
...
static IntermediateInstr * emitNewVar(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	...
	if (root->getChild(0) != nullptr) // Initializer expression node
		...
		argument = appendTrailingLoadOpIfNeeded(compiler, argument,
				root->getChild(0));
		argumentCount = 1;
	}

	findSymbolInfo(compiler, codeGen, root->symbol, &symbolType, &stackIndex,
			&symbolIsFuncLocal);
	const OpCode op = (
			symbolIsFuncLocal ?
					OpCode::StoreSetTypeLocal : OpCode::StoreSetTypeGlobal);

	IntermediateInstr * newOp = newInstruction(compiler, OpCode::NewVar,
			typeSymbol, Variant::Type::Tid, InvalidStackIndex);
	IntermediateInstr * storeOp = newInstruction(compiler, op, root->symbol,
			symbolType, stackIndex);
	...
	IntermediateInstr * loadArgCountInstr = makeLoadIntLiteral(compiler,
			argumentCount);

	if (argument != nullptr) {
		// Storing from the return value of a function references the Return Value Register (RVR).
		if (root->getChild(0)->nodeType == SyntaxTreeNode::Type::ExprFuncCall) {
			argument = linkInstructions(argument,
					newInstruction(compiler, OpCode::LoadRVR));
		}
...
static IntermediateInstr * emitLoad(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_SYMBOL(root);
	...
	if (symbol->type == Symbol::Type::BoolLiteral) {
		symbol = compiler.symTable.findOrDefineIntLiteral(
				symbol->value.asBoolean);
	} else if (symbol->type == Symbol::Type::Identifier) {
		bool symbolIsFuncLocal;
		Variant::Type symbolType;
		findSymbolInfo(compiler, codeGen, symbol, &symbolType, &stackIndex,
				&symbolIsFuncLocal);
	...
	IntermediateInstr * loadInstr = newInstruction(compiler, op, symbol,
			eval2VarType(root->evalType), stackIndex);

	// Function calls consist of a chain of load instructions followed by one CALL instruction.
	if (root->getChild(0) != nullptr) {
		IntermediateInstr * argList = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));
...
static IntermediateInstr * memberArrayStore(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * firstMemberRefNode,
		const SyntaxTreeNode * arraySubNode,
		const SyntaxTreeNode * assignmentNode) {
	LAVASCRIPT_ASSERT(firstMemberRefNode != nullptr); EXPECT_CHILD_AT_INDEX(arraySubNode, 2); EXPECT_CHILD_AT_INDEX(assignmentNode, 2);

	IntermediateInstr * memberRefInstrList = traverseTreeRecursive(compiler,
			codeGen, firstMemberRefNode);
	IntermediateInstr * assignValLoadInstr = traverseTreeRecursive(compiler,
			codeGen, assignmentNode->getChild(2));
	IntermediateInstr * arraySubLoadInstr = traverseTreeRecursive(compiler,
			codeGen, arraySubNode->getChild(2));

	// If any of the terms is a function call, we need the return value.
	if (assignmentNode->getChild(2)->nodeType
			== SyntaxTreeNode::Type::ExprFuncCall) {
		assignValLoadInstr = linkInstructions(assignValLoadInstr,
				newInstruction(compiler, OpCode::LoadRVR));
	}

	if (arraySubNode->getChild(2)->nodeType
			== SyntaxTreeNode::Type::ExprFuncCall) {
		arraySubLoadInstr = linkInstructions(arraySubLoadInstr,
				newInstruction(compiler, OpCode::LoadRVR));
	}
	...
	return linkInstructions(memberRefInstrList, assignValLoadInstr,
			arraySubLoadInstr,
			newInstruction(compiler, OpCode::StoreArraySubStk));
}

static IntermediateInstr * setUpMemberOffsetLoadChain(LSCompiler & compiler,
		CodeGenContext & codeGen, const Symbol * objSymbol) {
	LAVASCRIPT_ASSERT(objSymbol != nullptr);

	int lastMemberOffset = 0;
	const TypeId * lastMemberTypeId = nullptr;
	IntermediateInstr * memOffsetInstr = makeLoadMemberOffsetInstr(compiler,
			codeGen, objSymbol, codeGen.memberRefList[1], &lastMemberTypeId,
			&lastMemberOffset);

	if (codeGen.memberRefList.size() > 2) {
		const LSObject::Member & member =
				lastMemberTypeId->templateObject->getMemberAt(lastMemberOffset);
		if (member.data.type == Variant::Type::LSObject) {
		...
		for (std::size_t i = 2; i < numRefs; ++i) {
			LAVASCRIPT_ASSERT(lastMemberTypeId != nullptr);
			const Symbol * memberSymbol = codeGen.memberRefList[i];

			lastMemberOffset =
					lastMemberTypeId->templateObject->findMemberIndex(
							memberSymbol->name);
			if (lastMemberOffset < 0) {
				LAVASCRIPT_INTERNAL_EXCEPTION(
						"sub object of type '"
								+ toString(lastMemberTypeId->name)
								+ "' has no member '"
								+ toString(memberSymbol->name) + "'");
			}

			linkInstructions(memOffsetInstr,
					makeLoadIntLiteral(compiler, lastMemberOffset));

			const LSObject::Member & subMember =
					lastMemberTypeId->templateObject->getMemberAt(
							lastMemberOffset);
			if (subMember.data.type == Variant::Type::LSObject) {
...
static void collectMemberRefsRecursive(CodeGenContext & codeGen,
		const SyntaxTreeNode * root) {
	if (root == nullptr) {
		return;
	}
...
static IntermediateInstr * emitStore(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_CHILD_AT_INDEX(root, 1); EXPECT_CHILD_AT_INDEX(root, 2);
	...
	if (root->getChild(1)->nodeType == SyntaxTreeNode::Type::ExprMemberRef) {
		...
		collectMemberRefsRecursive(codeGen, root->getChild(1));
		LAVASCRIPT_ASSERT(codeGen.memberRefList.size() >= 2); // obj.member pair at least

		objSymbol = codeGen.memberRefList[0];
		numMemberOffsets = static_cast<int>(codeGen.memberRefList.size() - 1);
	} else if (root->getChild(1)->nodeType
			== SyntaxTreeNode::Type::ExprArraySubscript) {
		const SyntaxTreeNode * child11 = root->getChild(1)->getChild(1);
		LAVASCRIPT_ASSERT(child11 != nullptr);

		// Writing to a member array (obj.member[N] = ...)
		if (child11->nodeType == SyntaxTreeNode::Type::ExprMemberRef) {
			return memberArrayStore(compiler, codeGen, child11,
					root->getChild(1), root);
		} else {
	...
	LAVASCRIPT_ASSERT(objSymbol != nullptr);
	findSymbolInfo(compiler, codeGen, objSymbol, &symbolType, &stackIndex,
			&symbolIsFuncLocal);

	IntermediateInstr * memOffsetInstr = nullptr;
	IntermediateInstr * argument = traverseTreeRecursive(compiler, codeGen,
			root->getChild(2));
	...
	if (symbolIsFuncLocal) // A local variable or function parameter?
	{
		if (numMemberOffsets > 0) {
			memOffsetInstr = setUpMemberOffsetLoadChain(compiler, codeGen,
					objSymbol);
	...
	if (numMemberOffsets > 0) {
		LAVASCRIPT_ASSERT(memOffsetInstr != nullptr);
		memOffsetInstr = linkInstructions(memOffsetInstr,
				makeLoadIntLiteral(compiler, numMemberOffsets));
	}

	IntermediateInstr * storeInstr = newInstruction(compiler, op, objSymbol,
			symbolType, stackIndex);

	// Load the subscript right after if doing an array:
	if (isArraySubscript) {
		IntermediateInstr * subscriptInstr = traverseTreeRecursive(compiler,
				codeGen, root->getChild(1)->getChild(2));
		if (root->getChild(1)->getChild(2)->nodeType
				== SyntaxTreeNode::Type::ExprFuncCall) {
			// Subscript from function return value.
			IntermediateInstr * getRVR = newInstruction(compiler,
					OpCode::LoadRVR);
	...
	if (root->getChild(2)->nodeType == SyntaxTreeNode::Type::ExprFuncCall) {
		...
		return linkInstructions(linkInstructions(argument, memOffsetInstr),
				getRVR, storeInstr);
	} else // Normal store:
	{
		return linkInstructions(linkInstructions(argument, memOffsetInstr),
...
static IntermediateInstr * emitTypecast(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_CHILD_AT_INDEX(root, 1); EXPECT_CHILD_AT_INDEX(root, 2);

	// Second child must be a type identifier
	EXPECT_SYMBOL(root->getChild(2));
	IntermediateInstr * loadTypeIdOp = newInstruction(compiler,
			OpCode::LoadGlobal, root->getChild(2)->symbol, Variant::Type::Tid,
			InvalidStackIndex);
	...
	return linkInstructions(argument, loadTypeIdOp, typecastOp,
			getParamListChain(compiler, codeGen, root));
}

static IntermediateInstr * emitTypeof(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	EXPECT_CHILD_AT_INDEX(root, 1);
	const SyntaxTreeNode * child1 = root->getChild(1);

	const bool nodeIsTypeId = (child1->nodeType
			== SyntaxTreeNode::Type::ExprTypeIdent);
	const bool nodeIsLiteral = (child1->nodeType
			== SyntaxTreeNode::Type::ExprLiteralConst);
	const bool symbIsTypeId = (child1->symbol
			&& findTypeNameTypeId(codeGen, child1->symbol));
	const bool isNullLiteral = (nodeIsLiteral
			&& child1->evalType == SyntaxTreeNode::Eval::Null);
	...
	if (!isNullLiteral && (nodeIsTypeId || nodeIsLiteral || symbIsTypeId)) {
		const Symbol * typeSymbol = (
				nodeIsLiteral ?
						eval2Symbol(compiler.symTable, child1->evalType) :
						child1->symbol);
		typeofInstr = newInstruction(compiler, OpCode::LoadGlobal, typeSymbol,
	...
	} else // Resolve the argument expression first and emit a Typeof opcode:
	{
		...
		typeofInstr = linkInstructions(argument,
				newInstruction(compiler, OpCode::Typeof));
	}

	return linkInstructions(typeofInstr,
			getParamListChain(compiler, codeGen, root));
}

static void countArgsRecursive(CodeGenContext & codeGen,
		const SyntaxTreeNode * root, int * outArgCount) {
	if (root == nullptr) {
		return;
	}
...
static IntermediateInstr * emitParamChain(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	...
	if (op == OpCode::Call) {
		...
		bool symbolIsFuncLocal;
		Variant::Type symbolType;
		findSymbolInfo(compiler, codeGen, root->symbol, &symbolType,
				&stackIndex, &symbolIsFuncLocal);
	...
	IntermediateInstr * headInstr = newInstruction(compiler, op, root->symbol,
			OperandType, stackIndex);
	...
	if (root->getChild(0) != nullptr) {
		argListChain = traverseTreeRecursive(compiler, codeGen,
				root->getChild(0));
	}
	...
	IntermediateInstr * loadArgCountOp = makeLoadIntLiteral(compiler,
			argumentCount);
...
template<OpCode OP>
static IntermediateInstr * emitUnaryOp(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// child 0 is reserved for parameter list chains in function/constructor calls.
	EXPECT_CHILD_AT_INDEX(root, 1);
	IntermediateInstr * arg = traverseTreeRecursive(compiler, codeGen,
			root->getChild(1));
	...
	return linkInstructions(arg, unaryOp,
			getParamListChain(compiler, codeGen, root));
}

template<OpCode OP>
static IntermediateInstr * emitBinaryOp(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// child 0 is reserved for parameter list chains in function/constructor calls.
	EXPECT_CHILD_AT_INDEX(root, 1); EXPECT_CHILD_AT_INDEX(root, 2);
	IntermediateInstr * lhs = traverseTreeRecursive(compiler, codeGen,
	...
	return linkInstructions(lhs, rhs, binOp,
			getParamListChain(compiler, codeGen, root));
}

template<OpCode OP>
static IntermediateInstr * emitCompoundBinaryOp(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	// child 0 is reserved for parameter list chains in function/constructor calls.
	EXPECT_CHILD_AT_INDEX(root, 1); EXPECT_CHILD_AT_INDEX(root, 2);
	...
	findSymbolInfo(compiler, codeGen, lhsSymbol, &symbolType, &stackIndex,
			&symbolIsFuncLocal);
	...
	if (root->getChild(1)->nodeType != SyntaxTreeNode::Type::ExprFuncCall) {
		...
		IntermediateInstr * storeOpInstr = newInstruction(compiler, storeOp,
				lhsSymbol, symbolType, stackIndex);
		...
		IntermediateInstr * paramList = getParamListChain(compiler, codeGen,
				root);
		if (paramList != nullptr) {
			...
			paramList = linkInstructions(
					newInstruction(compiler, loadOp, lhsSymbol, symbolType,
							stackIndex), paramList);
		}

		return linkInstructions(lhs, rhs,
				linkInstructions(binOpInstr, memOffsetInstr), storeOpInstr,
				paramList);
	} else {
		...
		return linkInstructions(lhs, rhs, binOpInstr,
				getParamListChain(compiler, codeGen, root));
	}
}
...
// ----------------------------------------------------------------------------
static const EmitInstrForNodeCB emitInstrCallbacks[] { &emitNOOP,        // NoOp
...
static IntermediateInstr * traverseTreeRecursive(LSCompiler & compiler,
		CodeGenContext & codeGen, const SyntaxTreeNode * root) {
	LAVASCRIPT_ASSERT(root != nullptr);
	const auto handlerIndex = static_cast<UInt32>(root->nodeType);
	LAVASCRIPT_ASSERT(handlerIndex < static_cast<UInt32>(SyntaxTreeNode::Type::Count));
...
static UInt32 getProgCodeIndex(const InstrMap & instrMapping,
		const IntermediateInstr * const instr) {
	LAVASCRIPT_ASSERT(instr != nullptr);

	auto && entry = instrMapping.find(instr);
	if (entry == std::end(instrMapping)) {
		LAVASCRIPT_INTERNAL_EXCEPTION(
				"instruction " + toString(instr->op)
						+ " not found in InstructionMap");
	}
	return entry->second;
}

static UInt32 getProgDataIndex(const DataMap & dataMapping,
		const Symbol * const symbol) {
	LAVASCRIPT_ASSERT(symbol != nullptr);

	auto && entry = dataMapping.find(symbol);
	if (entry == std::end(dataMapping)) {
		LAVASCRIPT_INTERNAL_EXCEPTION(
				"symbol '" + toString(*symbol) + "' not found in DataMap");
	}
	return entry->second;
}

static UInt32 addSymbolToProgData(LSVM & vm, const Symbol * const symbol,
...
static void createMappings(LSCompiler & compiler, CodeGenContext & codeGen,
		LSVM & vm, IntermediateInstr * listHead, const bool skipFunctions) {
	...
	while (instr != nullptr) {
		...
		if (skipFunctions && instr->op == OpCode::FuncStart) {
			...
			for (; instr != nullptr && instr->op != OpCode::FuncEnd; instr =
					instr->next) {
			}
			if (instr == nullptr) {
				break;
		...
		if (instr->op != OpCode::NoOp) {
			...
			if (instr->operand.symbol != nullptr
					&& instr->stackIndex == InvalidStackIndex
					&& !isJumpOpCode(instr->op)) {
				// Each symbol is added once.
				if (codeGen.dataMapping.find(instr->operand.symbol)
						== codeGen.dataMapping.end()) {
					const auto index = addSymbolToProgData(vm,
							instr->operand.symbol, instr->dataType);
...
static void fixReferences(const IntermediateInstr * const instr,
		const CodeGenContext & codeGen, CodeVector & progCode,
		LSFunctionTable & funcTable) {
	LAVASCRIPT_ASSERT(instr != nullptr);
	switch (instr->op) {
	...
	case OpCode::FuncEnd: {
		...
		const UInt32 selfCodeIdx = getProgCodeIndex(codeGen.instrMapping,
				instr);
		const UInt32 operandDataIndex = getProgDataIndex(codeGen.dataMapping,
				instr->operand.symbol);
		LAVASCRIPT_ASSERT(selfCodeIdx < progCode.size() && "Index out-of-bounds!");
		...
		if (funcObj == nullptr) {
			LAVASCRIPT_INTERNAL_EXCEPTION(
					"missing function table entry for '" + toString(funcName)
							+ "'");
		}
	...
	case OpCode::JmpIfFalse: {
		...
		const UInt32 selfCodeIdx = getProgCodeIndex(codeGen.instrMapping,
				instr);
		UInt32 operandCodeIdx = getProgCodeIndex(codeGen.instrMapping,
				instr->operand.jumpTarget);
		...
		operandCodeIdx = std::min(operandCodeIdx,
				static_cast<UInt32>(progCode.size() - 1));
	...
	case OpCode::StoreSetTypeLocal: {
		...
		const UInt32 operandDataIndex = (
				(instr->stackIndex == InvalidStackIndex) ?
						getProgDataIndex(codeGen.dataMapping,
								instr->operand.symbol) :
						instr->stackIndex);
...
static void intermediateToVMBytecode(LSCompiler & compiler,
		CodeGenContext & codeGen, LSVM & vm) {
	...
	createMappings(compiler, codeGen, vm, compiler.globCodeListHead, /* skipFunctions = */
			true);
	...
	createMappings(compiler, codeGen, vm, compiler.funcCodeListHead, /* skipFunctions = */
			false);

	// And map back the instructions into the code vector:
	IntermediateInstr * instr;
	for (instr = compiler.globCodeListHead; instr != nullptr;
			instr = instr->next) {
		fixReferences(instr, codeGen, vm.code, vm.functions);
	}
	for (instr = compiler.funcCodeListHead; instr != nullptr;
			instr = instr->next) {
		fixReferences(instr, codeGen, vm.code, vm.functions);
	}
}
...
static void printIntermediateInstrListHelper(const IntermediateInstr * head,
		std::ostream & os) {
	os << color::white() << "[[ begin intermediate instruction dump ]]"
			<< color::restore() << "\n";

	int printedItems = 0;
	if (head != nullptr) {
		os << color::white() << "opcode count: "
				<< static_cast<UInt32>(OpCode::Count) << color::restore()
				<< "\n";

		for (const IntermediateInstr * instr = head; instr != nullptr; instr =
				instr->next) {
			...
			if (!isJumpOpCode(instr->op)) {
				if (instr->operand.symbol != nullptr) {
					if (instr->dataType != Variant::Type::Null) {
						os << " \'" << color::magenta()
								<< unescapeString(
										toString(*(instr->operand.symbol)).c_str())
								<< color::restore() << "\'" << " ("
								<< toString(instr->dataType) << ")"
								<< color::restore();
					} else {
						os << " \'" << color::magenta()
								<< unescapeString(
			...
			} else // Jump instruction. Operand is another instruction.
			{
				if (instr->operand.jumpTarget != nullptr) {
					os << " " << color::cyan() << instr->operand.jumpTarget->uid
							<< color::yellow() << " ("
							<< toString(instr->operand.jumpTarget->op) << ")"
							<< color::restore();
				} else // Null jump target. This would be an error in our compiler!
				{
					os << color::magenta() << "ERROR_NULL_JUMP_TARGET"
							<< color::restore();
				}
			}
...
static void printInstructionMappingHelper(const InstrMap & instrMapping,
		std::ostream & os) {
	os << color::white() << "[[ begin instruction map dump ]]"
			<< color::restore() << "\n";
	...
	os << color::white() << "[[ listed " << instrMapping.size()
			<< " instructions ]]" << color::restore() << "\n";
}
...
struct OpenScriptRAII final {
	using OpenMethod = bool (LSFileIOCallbacks::*)(const std::string &, std::istream **);

	LSFileIOCallbacks * callbacks;
	std::istream * stream;

	OpenScriptRAII(const std::string & filename, LSFileIOCallbacks * iocb,
			OpenMethod openMethod) :
			callbacks { iocb }, stream { doOpen(filename, openMethod) } {
	}
...
private:
	...
	std::istream * doOpen(const std::string & filename,
			OpenMethod openMethod) const {
		LAVASCRIPT_ASSERT(callbacks != nullptr);
...
class LSDefaultFileIOCallbacks final : public LSFileIOCallbacks {
public:
	LSDefaultFileIOCallbacks() = default;
	bool openScript(const std::string & scriptFile, std::istream ** streamOut)
			override;
	bool openScriptImport(const std::string & importFile,
			std::istream ** streamOut) override;
	void closeScript(std::istream ** stream) override;
};

bool LSDefaultFileIOCallbacks::openScriptImport(const std::string & importFile,
		std::istream ** streamOut) {
	// No extra handling. We don't perform import path searching for the default callbacks.
	return openScript(importFile, streamOut);
}

bool LSDefaultFileIOCallbacks::openScript(const std::string & scriptFile,
		std::istream ** streamOut) {
	LAVASCRIPT_ASSERT(!scriptFile.empty()); LAVASCRIPT_ASSERT(streamOut != nullptr);
...
#if LAVASCRIPT_DEBUG
	logStream() << "LavaScript: DefaultFileIOCallbacks - opened script file \"" << scriptFile << "\"\n";
#endif // LAVASCRIPT_DEBUG
	return succeeded;
}

void LSDefaultFileIOCallbacks::closeScript(std::istream ** stream) {
#if LAVASCRIPT_DEBUG
	logStream() << "LavaScript: DefaultFileIOCallbacks - closing script file.\n";
#endif // LAVASCRIPT_DEBUG
...
LSFileIOCallbacks::~LSFileIOCallbacks() {
...
static LSDefaultFileIOCallbacks g_defaultIOCallbacks { };

// ========================================================
// LSCompiler class methods:
// ========================================================

LSCompiler::LSCompiler() :
		LSCompiler { &g_defaultIOCallbacks } {
}

LSCompiler::LSCompiler(LSFileIOCallbacks * newIOCallbacks) {
	setFileIOCallbacks(newIOCallbacks);
}

void LSCompiler::parseScript(LSVM * vm, const std::string & filename,
		SyntaxTreeNode ** outTreeRoot) {
	LAVASCRIPT_ASSERT(vm != nullptr); LAVASCRIPT_ASSERT(!filename.empty());

	OpenScriptRAII script { filename, fileIOCallbacks,
			&LSFileIOCallbacks::openScript };
	if (!script.isOpen()) {
		LAVASCRIPT_INTERNAL_EXCEPTION(
...
void LSCompiler::parseScriptImport(LSVM * vm, const std::string & filename,
		SyntaxTreeNode ** outImportRoot) {
	LAVASCRIPT_ASSERT(vm != nullptr); LAVASCRIPT_ASSERT(!filename.empty());

#if LAVASCRIPT_DEBUG
	logStream() << "LavaScript: Importing script \"" << filename << "\"...\n";
#endif // LAVASCRIPT_DEBUG

	OpenScriptRAII script { filename, fileIOCallbacks,
			&LSFileIOCallbacks::openScriptImport };
	if (!script.isOpen()) {
		LAVASCRIPT_INTERNAL_EXCEPTION(
				"unable to open script import \"" + filename + "\"");
	}

	parseScript(vm, script.stream, filename, outImportRoot);
}

void LSCompiler::parseScript(LSVM * vm, std::istream * source,
		const std::string & filename, SyntaxTreeNode ** outTreeRoot) {
	LAVASCRIPT_ASSERT(vm != nullptr); LAVASCRIPT_ASSERT(source != nullptr);
	...
	if (finishedOK) {
		...
		LAVASCRIPT_INTERNAL_EXCEPTION(
				"error while parsing the source code for \"" + filename + "\"");
	}
}

void LSCompiler::compile(LSVM * vm) {
	LAVASCRIPT_ASSERT(vm != nullptr);

	CodeGenContext codeGen;
	codeGen.vm = vm;

	// Traverse the syntax tree to generate a list of intermediate instructions:
	globCodeListHead = traverseTreeRecursive(*this, codeGen,
			syntTree.getRoot());
...
void LSCompiler::setFileIOCallbacks(LSFileIOCallbacks * newIOCallbacks) noexcept
{
...
void LSCompiler::print(std::ostream & os) const {
