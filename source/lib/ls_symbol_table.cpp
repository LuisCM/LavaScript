...
{
	LAVASCRIPT_ASSERT(otherName != nullptr);
...
Symbol::Value Symbol::valueFromIntegerStr(const char * cstr)
{
    ...
    {
        LAVASCRIPT_RUNTIME_EXCEPTION("string literal \"" + toString(cstr) +
...
Symbol::Value Symbol::valueFromBoolStr(const char * cstr)
{
    ...
    {
        LAVASCRIPT_RUNTIME_EXCEPTION("bad boolean literal string \"" + toString(cstr) +
...
const Symbol * LSSymbolTable::addSymbol(ConstRcString * name, const int declLineNum, const Symbol::Type type, const Symbol::Value value)
{
    LAVASCRIPT_ASSERT(!findSymbol(name) && "Symbol already registered!");
    return addInternal(name, construct(symbolPool.allocate(), { name, value, declLineNum, type }));
}

const Symbol * LSSymbolTable::addSymbol(const char * const name, const int declLineNum, const Symbol::Type type, const Symbol::Value value)
{
    ConstRcStrUPtr key{ newConstRcString(name) };
    return addSymbol(key.get(), declLineNum, type, value);
}

const Symbol * LSSymbolTable::findOrDefineLiteral(ConstRcString * valueStr, const int declLineNum, const Symbol::Type typeWanted)
{
    LAVASCRIPT_ASSERT(isRcStringValid(valueStr));
    LAVASCRIPT_ASSERT(typeWanted != Symbol::Type::Identifier && "Use LSSymbolTable::findOrDefineIdentifier() instead!");
...
const Symbol * LSSymbolTable::findOrDefineLiteral(const char * const valueStr, const int declLineNum, const Symbol::Type typeWanted)
{
    LAVASCRIPT_ASSERT(valueStr != nullptr);
    LAVASCRIPT_ASSERT(typeWanted != Symbol::Type::Identifier && "Use LSSymbolTable::findOrDefineIdentifier() instead!");
...
const Symbol * LSSymbolTable::findOrDefineIdentifier(ConstRcString * name, const int declLineNum)
{
    if (auto symbol = findSymbol(name))
    {
        LAVASCRIPT_ASSERT(symbol->type == Symbol::Type::Identifier);
        return symbol;
    }
...
const Symbol * LSSymbolTable::findOrDefineIdentifier(const char * const name, const int declLineNum)
{
    if (auto symbol = findSymbol(name))
    {
        LAVASCRIPT_ASSERT(symbol->type == Symbol::Type::Identifier);
...
const Symbol * LSSymbolTable::findOrDefineIntLiteral(const Int64 value)
{
    char valueStr[128];
    std::snprintf(valueStr, arrayLength(valueStr), LAVASCRIPT_I64_PRINT_FMT, value);
    return findOrDefineLiteral(valueStr, Symbol::LineNumBuiltIn, Symbol::Type::IntLiteral);
}

const Symbol * LSSymbolTable::findOrDefineFloatLiteral(const Float64 value)
{
    char valueStr[128];
...
const Symbol * LSSymbolTable::findOrDefineBoolLiteral(const bool value)
{
    return findOrDefineLiteral((value ? "true" : "false"), Symbol::LineNumBuiltIn, Symbol::Type::BoolLiteral);
}

const Symbol * LSSymbolTable::findOrDefineStrLiteral(const char * const valueStr)
{
    return findOrDefineLiteral(valueStr, Symbol::LineNumBuiltIn, Symbol::Type::StrLiteral);
}

ConstRcStrUPtr LSSymbolTable::generateSymbolName(const Symbol::Type type)
{
    char tempStr[128];
    switch (type)
    {
...
void LSSymbolTable::print(std::ostream & os) const
{
