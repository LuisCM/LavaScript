...
[[noreturn]] static void scriptError(const Int64 lineNum,
		const char * srcFileName, const char * callerName,
		const char * colorCode, const char * baseMessage,
	...
	lavascript::scriptError(message);
}

void assert(LSVM &, LSStack::LSSlice args) // (varargs) -> void
		{
	...
	LAVASCRIPT_ASSERT(fileName != nullptr && lineNum != nullptr && funcName != nullptr);
	...
	const char * callerName = (
			(funcName->type == Variant::Type::Function) ?
	...
	scriptError(lineNum->getAsInteger(), fileName->getAsString()->c_str(),
			callerName, color::yellow(), "script assert() failed: ",
			userMessage);
}

void panic(LSVM &, LSStack::LSSlice args) // (varargs) -> void
		{
...
void print(LSVM &, LSStack::LSSlice args) // (varargs) -> void
		{
	for (const Variant * var = args.first(); var != nullptr; var =
			args.next()) {
		logStream() << toString(*var);
	}
}

void println(LSVM &, LSStack::LSSlice args) // (varargs) -> void
		{
	for (const Variant * var = args.first(); var != nullptr; var =
			args.next()) {
		logStream() << toString(*var);
...
void to_string(LSVM & vm, LSStack::LSSlice args) // (varargs) -> string
		{
	if (args.getSize() < 1) {
		LAVASCRIPT_RUNTIME_EXCEPTION(
				"to_string() requires at least one argument");
	}
	...
	LAVASCRIPT_ASSERT(var != nullptr);
	if (fmt != nullptr) {
		if (fmt->type != Variant::Type::LSStr || fmt->value.asString == nullptr) {
			LAVASCRIPT_RUNTIME_EXCEPTION(
					"to_string() format flag should be a string, "
							"e.g.: \"b\", \"f\", \"h\", \"p\", etc");
		}
		...
		case 'c': // character
		{
			if (var->type == Variant::Type::Integer) {
				outputStr = strPrintF("%c",
						static_cast<char>(var->value.asInteger));
			} else {
				outputStr = "?";
			}
			break;
		...
		case 'h': // hexadecimal
		{
			...
			outputStr = strPrintF(LAVASCRIPT_X64_PRINT_FMT, value);
			break;
		}
		case 'p': // pointer
		{
			...
			if (var->type == Variant::Type::Integer
				...
				address =
						static_cast<UInt64>(reinterpret_cast<std::uintptr_t>(var));
				outputStr = strPrintF(LAVASCRIPT_X64_PRINT_FMT, address);
			} else // Objects print the address of the object:
			{
				address =
						static_cast<UInt64>(reinterpret_cast<std::uintptr_t>(var->value.asVoidPtr));
				outputStr = strPrintF(LAVASCRIPT_X64_PRINT_FMT, address);
			}
			break;
		}
		...
		{
			if (var->type == Variant::Type::LSObject
		...
		default:
			LAVASCRIPT_RUNTIME_EXCEPTION(
					"to_string() called with unrecognized format flag '"
							+ toString(flag) + "'");
		} // switch (*flag)
	} else {
	...
	Variant result { Variant::Type::LSStr };
	result.value.asString = LSStr::newFromString(vm, outputStr, /* makeConst = */
			false);
	vm.setReturnValue(result);
}

void gc_need_to_collect(LSVM & vm, LSStack::LSSlice) // () -> bool
		{
...
void gc_collect(LSVM & vm, LSStack::LSSlice) // () -> void
		{
...
void registerNativeBuiltInFunctions(LSFunctionTable & funcTable) {
	const auto retStr = Variant::Type::LSStr;
	const auto retInt = Variant::Type::Integer;

	ADD_VARARGS_FUNC(funcTable, nullptr, assert,
			Function::VarArgs | Function::AddCallerInfo | Function::DebugOnly);
	ADD_VARARGS_FUNC(funcTable, nullptr, panic,
			Function::VarArgs | Function::AddCallerInfo);
