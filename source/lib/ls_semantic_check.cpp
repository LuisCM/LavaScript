...
void LSVarInfoTable::beginFunc(ParseContext & ctx, const Symbol * funcNameSymbol)
{
...
void LSVarInfoTable::endFunc(ParseContext & ctx)
{
...
void LSVarInfoTable::beginUDT(ParseContext & ctx, const Symbol * typeNameSymbol, const char * elementName)
{
    requireGlobalScope(ctx, elementName);
    currentUDTSymbol = typeNameSymbol;
}

void LSVarInfoTable::endUDT(ParseContext & /* ctx */)
{
    currentUDTSymbol = nullptr;
}

void LSVarInfoTable::beginForLoop(ParseContext & ctx, const Symbol * iteratorVarSymbol, const SyntaxTreeNode * iterableNode)
{
    if (forLoopIteratorNode != nullptr)
    {
        if (forLoopIterStackTop >= MaxNestedForLoops)
        {
            LAVASCRIPT_INTERNAL_EXCEPTION("too many nested for loops!"); // Some nasty loops :P
        }
...
void LSVarInfoTable::endForLoop(ParseContext & /* ctx */)
{
...
const VarInfo * LSVarInfoTable::findVar(const Symbol * symbol) const
{
...
const VarInfo * LSVarInfoTable::findFuncLocalVar(const Symbol * symbol) const
{
    LAVASCRIPT_ASSERT(symbol != nullptr);
    LAVASCRIPT_ASSERT(currentFuncLocals != nullptr);
...
const VarInfo * LSVarInfoTable::findNamedFuncLocalVar(const Symbol * varSymbol, const Symbol * funcSymbol)
{
...
const VarInfo * LSVarInfoTable::findGlobalVar(const Symbol * symbol) const
{
    LAVASCRIPT_ASSERT(symbol != nullptr);
...
void LSVarInfoTable::addGlobalVar(const Symbol * varNameSymbol, VarInfo vi)
{
    LAVASCRIPT_ASSERT(varNameSymbol != nullptr);

    vi.stackIndex = InvalidStackIndex;
    globalVars.insert(std::make_pair(varNameSymbol, vi));
}

void LSVarInfoTable::addFuncLocalVar(const Symbol * varNameSymbol, VarInfo vi)
{
    LAVASCRIPT_ASSERT(varNameSymbol     != nullptr);
    LAVASCRIPT_ASSERT(currentFuncLocals != nullptr);
    LAVASCRIPT_ASSERT(currentStackSize  != nullptr);

    // Up to 64K function parameters + local vars.
    if (*currentStackSize >= UINT16_MAX)
    {
        LAVASCRIPT_INTERNAL_EXCEPTION("maximum number of local variables and function parameters exceeded in '" +
...
int LSVarInfoTable::getFuncStackSize(const Symbol * funcSymbol)
{
    LAVASCRIPT_ASSERT(funcSymbol != nullptr);
...
bool LSVarInfoTable::removeVar(const Symbol * symbol)
{
    bool removed;
    if (isParsingFunction())
    {
...
bool LSVarInfoTable::removeGlobalVar(const Symbol * symbol)
{
    LAVASCRIPT_ASSERT(symbol != nullptr);
    return globalVars.erase(symbol) != 0;
}

bool LSVarInfoTable::removeFuncLocalVar(const Symbol * symbol)
{
    LAVASCRIPT_ASSERT(symbol != nullptr);
    LAVASCRIPT_ASSERT(currentFuncLocals != nullptr);
...
{
    LAVASCRIPT_ASSERT(ctx.compiler    != nullptr);
    LAVASCRIPT_ASSERT(ctx.importTable != nullptr);
...
static void registerVariable(ParseContext & ctx, const Symbol * varNameSymbol,
                             ...
                             const STEval stEval, void (LSVarInfoTable::*pAddVarMethod)(const Symbol *, VarInfo))
{
    ...
    {
        LAVASCRIPT_ASSERT(tidNode != nullptr && tidNode->symbol != nullptr);
    ...
    const STEval arrayEval = ((varType == Variant::Type::LSArray && initNode != nullptr) ?
        ...
        initNode->symbol != nullptr && initNode->nodeType != STNode::ExprFuncCall)
    {
        ...
        LAVASCRIPT_ASSERT(funcPtr != nullptr); // Already syntax-checked, so this suffices.
    }
                                    ...
                                    const SyntaxTreeNode * typeNode, STEval stEval)
{
    if (!ctx.varInfo->isParsingFunction()) // Global scope or parsing a function declaration?
    {
        ...
        registerVariable(ctx, varNameSymbol, initNode, typeNode, stEval, &LSVarInfoTable::addGlobalVar);
    }
    else // Function scope
    {
        if (stEval == STEval::Undefined)
        ...
        registerVariable(ctx, varNameSymbol, initNode, typeNode, stEval, &LSVarInfoTable::addFuncLocalVar);
    }
...
SyntaxTreeNode * newIdentNode(ParseContext & ctx, const Symbol * identifierSymbol)
{
    ...
    static constexpr auto fileHash = ct::hashCString("LS_SRC_FILE_NAME");
    static constexpr auto lineHash = ct::hashCString("LS_SRC_LINE_NUM");
    static constexpr auto funcHash = ct::hashCString("LS_SRC_FUNC_NAME");
                                       ...
                                       const SyntaxTreeNode * indexExpr)
{
    ...
    LAVASCRIPT_ASSERT(arrayExpr->symbol != nullptr);
    ...
    else
    {
        ...
        LAVASCRIPT_ASSERT(vi->arrayEval != STEval::Undefined);
        LAVASCRIPT_ASSERT(vi->varType == Variant::Type::Array);
                                 ...
                                 SyntaxTreeNode * lhs, SyntaxTreeNode * rhs)
{
    ...
    default : LAVASCRIPT_INTERNAL_EXCEPTION("unhandled STNode type " + toString(type) + " in binary op!");
    } // switch (type)
    ...
    if (binOp == OpCode::MemberRef) // Validate the object and member types
    {
        ...
        {
            LAVASCRIPT_ASSERT(lhs->symbol != nullptr);
        ...
        if (member.type == Variant::Type::LSObject)
        {
            LAVASCRIPT_ASSERT(member.getAsObject() != nullptr);
...
SyntaxTreeNode * newTypecastNode(ParseContext & ctx, const SyntaxTreeNode * lhs, const SyntaxTreeNode * rhs)
{
    ...
    LAVASCRIPT_ASSERT(rhs->nodeType == STNode::ExprTypeIdent);
...
SyntaxTreeNode * newTypeofNode(ParseContext & ctx, SyntaxTreeNode * exprOrTypeNode)
{
    ...
    if (stEval == STEval::Undefined)
    {
        ...
        LAVASCRIPT_ASSERT(exprOrTypeNode->symbol != nullptr);
        ...
        {
            parserError(ctx, "undefined type or expression as typeOf() argument");
        }
                                        ...
                                        const Symbol * iteratorVarSymbol)
{
    ...
    LAVASCRIPT_ASSERT(ctx.lexer   != nullptr);
    LAVASCRIPT_ASSERT(ctx.varInfo != nullptr);
                             ...
                             const SyntaxTreeNode * bodyStatementListNode)
{
    ...
    else if (nodeType == STNode::ForStatement) // 'for i in range do'
    {
        ...
        LAVASCRIPT_ASSERT(ctx.varInfo->forLoopIteratorNode != nullptr);
    ...
    {
        LAVASCRIPT_INTERNAL_EXCEPTION("bad loop node type!");
    }
}
...
{
    LAVASCRIPT_ASSERT(funcNameSymbol != nullptr);
    ...
    const LSStack::LSSlice args{ argVars, static_cast<int>(argCount) };
...
static void collectObjMembers(ParseContext & ctx, const SyntaxTreeNode * root, LSObject * objOut)
{
    if (root == nullptr)
    {
        return;
    ...
    if (data.type == Variant::Type::LSObject)
    {
        LAVASCRIPT_ASSERT(root->getChild(1) != nullptr);
        ...
        data.value.asObject = const_cast<LSObject *>(typeId->templateObject);
    }
                                   ...
                                   const SyntaxTreeNode * structMembersListNode)
{
    ...
    LSObject * templateObj = LSStruct::newInstance(*ctx.vm, nullptr);
    const TypeId * newTypeId = ctx.vm->types.addTypeId(typeName, &LSStruct::newInstance, templateObj, false);
...
static void initEnumMembers(ParseContext & ctx, const SyntaxTreeNode * root,
                            LSObject * objOut, Int64 & nextEnumIntVal)
{
    if (root == nullptr)
    {
        return;
    ...
    LAVASCRIPT_ASSERT(objOut != nullptr);
    LAVASCRIPT_ASSERT(root->symbol != nullptr);
    LAVASCRIPT_ASSERT(root->symbol->name != nullptr);
    ...
    if (initExprNode != nullptr)
    {
        ...
        {
            LAVASCRIPT_ASSERT(initExprNode->symbol != nullptr);

            if (initExprNode->evalType == STEval::Int ||
                initExprNode->evalType == STEval::Long)
            {
                LAVASCRIPT_ASSERT(data.type == Variant::Type::Integer);
            ...
            else // string, float, bool
            {
                ...
                LAVASCRIPT_ASSERT(data.type == initVal.type);
                data = initVal;
                // Enum strings have program lifetime.
                if (data.type == Variant::Type::LSStr)
                {
        ...
        else // identifier / other enum member ref
        {
            ...
            {
                LAVASCRIPT_ASSERT(initExprNode->nodeType == STNode::ExprMemberRef);

                // Already checked when parsing, so an assert suffices.
                LAVASCRIPT_ASSERT(initExprNode->getChildSymbol(1) != nullptr);
                ...
                LAVASCRIPT_ASSERT(otherEnum != nullptr);

                LAVASCRIPT_ASSERT(initExprNode->getChildSymbol(2) != nullptr);
...
const Symbol * mangleEnumTypeName(LSSymbolTable & symTable, const Symbol * enumNameSymbol, int declLineNum)
{
...
const Symbol * demangleEnumTypeName(LSSymbolTable & symTable, const Symbol * enumNameSymbol)
{
...
SyntaxTreeNode * newEnumDeclNode(ParseContext & ctx, const Symbol * enumNameSymbol, SyntaxTreeNode * enumConstantsListNode)
{
    ...
    LSObject * templateObj = LSEnum::newInstance(*ctx.vm, nullptr);
    const TypeId * newTypeId = ctx.vm->types.addTypeId(typeName, &LSEnum::newInstance, templateObj, false);
    templateObj->setTypeId(newTypeId);
    templateObj->markTypeTemplate();
...
{
    LAVASCRIPT_ASSERT(eTypeNode->symbol  != nullptr);
    LAVASCRIPT_ASSERT(eConstNode->symbol != nullptr);
    ...
    LAVASCRIPT_ASSERT(enumType->templateObject != nullptr);
...
STEval varType2Eval(const Variant::Type varType)
{
    ...
    case Variant::Type::LSStr    : return STEval::Str;
    case Variant::Type::LSObject : return STEval::UDT;
    case Variant::Type::LSArray  : return STEval::Array;
    case Variant::Type::Range    : return STEval::Range;
    case Variant::Type::Any      : return STEval::Any;
    default : LAVASCRIPT_INTERNAL_EXCEPTION("can't convert Variant type '" + toString(varType) + "' to STEval!");
    } // switch (varType)
}
...
Variant::Type eval2VarType(const STEval stEval)
{
    ...
    case STEval::VarArgs : return Variant::Type::LSArray;
    ...
    case STEval::Str     : return Variant::Type::LSStr;
    case STEval::Array   : return Variant::Type::LSArray;
    ...
    case STEval::UDT     : return Variant::Type::LSObject;
    default : LAVASCRIPT_INTERNAL_EXCEPTION("can't convert STEval '" + toString(stEval) + "' to Variant type!");
    } // switch (stEval)
}

const TypeId * varType2TypeId(const LSTypeTable & typeTable, const Variant::Type varType)
{
    switch (varType)
    {
    ...
    case Variant::Type::LSStr    : return typeTable.strTypeId;
    case Variant::Type::LSObject : return typeTable.objectTypeId;
    case Variant::Type::LSArray  : return typeTable.arrayTypeId;
...
const TypeId * eval2TypeId(const LSTypeTable & typeTable, const STEval stEval)
{
    switch (stEval)
    {
    ...
    default : LAVASCRIPT_INTERNAL_EXCEPTION("can't convert STEval '" + toString(stEval) + "' to TypeId!");
    } // switch (stEval)
}
...
STEval symbol2Eval(const Symbol & symbol)
{
    ...
    default : LAVASCRIPT_INTERNAL_EXCEPTION("can't convert symbol '" + toString(symbol.type) + "' to STEval!");
    } // switch (symbol.type)
}

const Symbol * eval2Symbol(const LSSymbolTable & symTable, const STEval stEval)
{
    ...
    default : LAVASCRIPT_INTERNAL_EXCEPTION("can't convert STEval '" + toString(stEval) + "' to a symbol!");
    } // switch (stEval)
}

Variant symbol2Variant(LSVM & vm, const Symbol & symbol)
{
    Variant var;
    switch (symbol.type)
    {
    ...
    case Symbol::Type::StrLiteral :
        var.type = Variant::Type::LSStr;
        var.value.asString = LSStr::newFromString(vm, symbol.value.asString);
        break;
    default :
        LAVASCRIPT_RUNTIME_EXCEPTION("symbol '" + toString(symbol) + "' doesn't map directly to a Variant!");
    } // switch (symbol.type)
    return var;
}
...
Variant::Type typeId2VarType(const TypeId * tid)
{
    ...
    if (ct::hashCString( "object"   ) == hashId) { return Variant::Type::LSObject; }
    if (ct::hashCString( "function" ) == hashId) { return Variant::Type::Function; }
    if (ct::hashCString( "str"      ) == hashId) { return Variant::Type::LSStr;    }
    if (ct::hashCString( "array"    ) == hashId) { return Variant::Type::LSArray;  }
    if (ct::hashCString( "tid"      ) == hashId) { return Variant::Type::Tid;      }

    LAVASCRIPT_RUNTIME_EXCEPTION("bad variable type id '" + toString(tid->name) +
