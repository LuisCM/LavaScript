...
class LSVarInfoTable final
{
public:

    LSVarInfoTable() = default;
    LSVarInfoTable(const LSVarInfoTable &) = delete;
    LSVarInfoTable & operator = (const LSVarInfoTable &) = delete;
...
class LSImportTable final
{
public:

    LSImportTable() = default;
    LSImportTable(const LSImportTable &) = delete;
    LSImportTable & operator = (const LSImportTable &) = delete;
    ...
    {
        LAVASCRIPT_ASSERT(isRcStringValid(filename));
...
const TypeId * eval2TypeId(const LSTypeTable & typeTable, const SyntaxTreeNode::Eval stEval);

SyntaxTreeNode::Eval symbol2Eval(const Symbol & symbol);
const Symbol * eval2Symbol(const LSSymbolTable & symTable, SyntaxTreeNode::Eval stEval);

Variant symbol2Variant(LSVM & vm, const Symbol & sym);
...
#endif // LAVASCRIPT_SEMANTIC_CHECK_HPP
