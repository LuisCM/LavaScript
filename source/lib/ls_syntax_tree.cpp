...
SyntaxTreeNode * LSSyntaxTree::newNode(const SyntaxTreeNode::Type type,
                                     const SyntaxTreeNode * child0,
                                     const SyntaxTreeNode * child1,
                                     const SyntaxTreeNode * child2,
...
SyntaxTreeNode * LSSyntaxTree::newNodeWithSymbol(const SyntaxTreeNode::Type type,
                                               const Symbol * symbol,
                                               const SyntaxTreeNode * child0,
                                               const SyntaxTreeNode * child1,
                                               const SyntaxTreeNode * child2,
...
SyntaxTreeNode * LSSyntaxTree::newNodeWithEval(const SyntaxTreeNode::Type type,
                                             const Symbol * symbol,
                                             const SyntaxTreeNode::Eval eval)
{
...
void LSSyntaxTree::print(std::ostream & os) const
{
...
void LSSyntaxTree::checkNodeRecursive(const SyntaxTreeNode * const node)
{
    if (node == nullptr)
    {
        return;
    ...
    {
        LAVASCRIPT_INTERNAL_EXCEPTION("invalid node type detected!");
    }

    if (static_cast<UInt32>(node->evalType) >= static_cast<UInt32>(SyntaxTreeNode::Eval::Count))
    {
        LAVASCRIPT_INTERNAL_EXCEPTION("invalid node eval detected!");
    }
    ...
    if (node->evalType == SyntaxTreeNode::Eval::Undefined)
    {
        ...
        {
            LAVASCRIPT_INTERNAL_EXCEPTION("eval of node '" + toString(node->nodeType) + "' ('" +
                                    toString(node->symbol->name) + "') is undefined");
        }
        else
        {
            LAVASCRIPT_INTERNAL_EXCEPTION("eval of node '" + toString(node->nodeType) + "' is undefined");
        }
    }
...
void LSSyntaxTree::validateNodes() const
{
